...
if(isset($_POST) && $_POST['submit']=='ADD') {
	 ...
	 if($name!='')
	{
		...
		//$ans = str_replace('"',"~~", $an);
		//echo "INSERT INTO `past_conferences`(`name`,`date`,`description`,`image`) VALUES ('$name','$designation','$ans','$image')";exit;
		 $sth = $db->query("INSERT INTO `past_conferences`(`name`,`image`) VALUES ('$name','$image')");
	}
	if($sth == true)  { ?>
   ...
   if(!empty($filename)) 
	{
		...
		move_uploaded_file($_FILES['image']['tmp_name'],"../adminupload/".$image);
	        ...
	        $a1 = $db->query("UPDATE `past_conferences` SET `image`= '$image' WHERE `guid`='$qid'");
	}
	...
	$sth = $db->query("UPDATE `past_conferences` SET `name`= '$name' WHERE `guid`='$qid'");
	}
              ...
              <div class="card-body">
                <form method="post" action="<?php echo $_SEVER['PHP_SELF'];?>" enctype="multipart/form-data" onsubmit="return validate();">
                  ...
                  <!--div class="form-group">
				   ...
				   <div class="form-group">
                    <label class="control-label">Date and Location</label>
                    <input type="text" name="designation" placeholder="Enter Date and Location" id="designation" class="form-control">
                  </div-->
              ...
              <div class="card-body">
                <form method="post" action="<?php echo $_SEVER['PHP_SELF'];?>" enctype="multipart/form-data">
				 ...
				 <div class="form-group">
                  ...
                  <div class="form-group">
                    <label class="control-label">Date and Location</label>
                    <input type="text" name="designation" id="designation" class="form-control" value="<?php echo $row[date];?>">
                  </div-->
