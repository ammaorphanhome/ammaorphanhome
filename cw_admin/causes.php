    ...
    if(isset($_POST) && $_POST['submit']=='ADD') {
        ...
        <?php
            ...
            if ($time) {
                ...
                <script type="text/javascript">
                    alert('Invalid date selected, Please try Again');
              		window.location="<?php echo URL; ?>causes.php";
    		 	</script>
            <?php }
        	...
        	if($shortDescription !='' && $description!= ''){
                ...
                $sth = $db->query("INSERT INTO `causes` (`title`, `short_description`, `image`, `description`, `date`) 
    	       ...
    	       if($sth == true)  { ?> 
    	    		<script type="text/javascript">
              			alert('Latest Needs details successfully added');
              			window.location="<?php echo URL; ?>causes.php";
            		</script>
    	    <?php } else { ?>
    	    		<script type="text/javascript">
      					alert('Latest Needs are not added, Please try Again');
              			window.location="<?php echo URL; ?>causes.php";
    		 		</script>
    	    <?php } ?>
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        <div class="page-title">
          ...
          <div><h1>Enter Latest Needs</h1></div>
			
		<div>
            <ul class="breadcrumb">
              <li><i class="fa fa-home fa-lg"></i></li>
              <li class="active"><a href="causes.php">Latest Home Needs </a></li>
            </ul>
          </div>
	  ...
	  <?php if(isset($action) && $action=='add') { ?>
	  		<a href="<?php echo URL; ?>causes.php" class="btn btn-info">Back</a>
			<div class="row">
				<div class="col-md-6">
					<div class="card">
						<h3 class="card-title">Add Latest Needs Details</h3>
						...
						<div class="card-body">
							<form role="form" method="post"
								...
								<div class="form-group">
                                    <label class="control-label">Ttile of the Latest Need:</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="shortDescription" placeholder="Enter Short Decription to display on home page's needs section" id="shortDescription" class="form-control" required>
                                </div>
                                
                                <div class="form-group" class="control-label">
                                    <label for="message">Note</label>
                                    <textarea cols="30" rows="8" class="form-control" id="description" name="description"
                                              placeholder="Enter Decription for Latest Needs"></textarea>
                                </div>
	  ...
	  <?php  if(isset($action) && $action=='viewDetails') { 
	       $q = $_GET['guid'];
	       $sth = $db->query ('SELECT * FROM `causes` WHERE `guid`="'.$q.'"');
	       $row = $sth->fetch();
	  ?>
		...
		else if(isset($_POST) && $_POST['submit']=='Edit')
		  {
              ...
              if(!empty($filename)) {
		          ...
		          $sth = $db->query("UPDATE `causes` SET `title`='$title' , `date`='$new_date', `short_description`='$shortDescription' , `description`='$description' , `image`='$image'  WHERE `guid`='$qid'");
		          ...
		          $sth = $db->query("UPDATE `causes` SET `title`='$title' , `date`='$new_date', `short_description`='$shortDescription' , `description`='$description'  WHERE `guid`='$qid'");
		    ...
		    $sth = $db->query ('SELECT * FROM `causes` WHERE `guid`="'.$guid.'"');
		      $row = $sth->fetch();
		      ?>
    		    <a href="<?php echo URL; ?>causes.php?id=<?php echo $guid; ?>" class="btn btn-info">Back</a>
                <div class="row">
        		<div class="col-md-6">
                    <div class="card">
                      <h3 class="card-title">Edit Latest Needs</h3>
                      <div class="card-body">
		  ...
		  <?php }
	       ...
	       $q = $_GET['guid'];
	       $vth = $db->query ("DELETE FROM `causes` WHERE `guid`='".$q."'" );
	       header('location:'.URL.'causes.php'); 
          ...
          <div class="col-md-12">
		  <div>
			 <a href="causes.php?action=add&id=<?php echo $_GET[id];?>"
				class="btn btn-success"><i class="fa fa-lg fa-plus"></i>Add New</a>
		  </div>
					
		  <?php
            $sth = $db->query ("SELECT * FROM `causes` ORDER BY `guid` DESC");
			$count = $sth->rowCount(); ?>
            <div class="card">
              <div class="card-body">
                <table id="sampleTable" class="table table-hover table-bordered">
				  ...
				  <?php 
                    ...
                    <tr>
					  ...
					  <td class="center"><div class="btn btn-xs btn-group-xs"><a href="causes.php?action=viewDetails&guid=<?php echo $row[guid]; ?>"  class="btn btn-warning"><i class="fa fa-eye icon-only"></i></a>  </div></td>
					  
					  <td class="center"><div class="btn btn-xs btn-group-xs"><a href="<?php echo URL; ?>causes.php?action=edit&guid=<?php echo $row[guid]; ?>" class="btn btn-primary icon-btn">Edit </a> <a href="<?php echo URL; ?>causes.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> </div></td>
                    </tr>
	  				<?php $m++; } } ?>
                  </tbody>
                </table>
