...
if(isset($_POST) && $_POST['submit']=='ADD') {
	...
	for($i=0;$i<sizeof($_POST['name']);$i++){
		...
		//echo "INSERT INTO `tutorial`(`id`,`name`) VALUES ('$id','".$_POST[name][$i]."')";exit;
		 ...
		 $sth = $db->query("INSERT INTO `curriculum`(`id`,`name`,`dur`) VALUES ('$id','$ans','$ans1')");
	}
	if($sth == true)  { ?>
  	        <script type="text/javascript">
  			alert('Data Successfully Inserted');
  			window.location="curriculum.php?id=<?php echo $id; ?>";
  			</script>
  ...
  if(isset($_POST) && $_POST['submit']=='EDIT')
  {
   ...
   // if(!empty($filename)) 
	//{
	        ...
	        //$a1 = $db->query("UPDATE `curriculum` SET `image`= '$image' WHERE `guid`='$qid'");
	// }
	...
	if($name!='')
	{
		...
		//echo $refid;exit;
		//echo "UPDATE `curriculum` SET `description`= '$ans' WHERE `guid`='$guid'";exit;
		//echo "UPDATE `curriculum` SET `name`= '$name',`sbttl`= '$designation',`wbf`= '$ans' WHERE `guid`='$qid'";exit;
	$sth = $db->query("UPDATE `curriculum` SET `name`= '$ans',`dur`='$ans1' WHERE `guid`='$qid'");
	}
	
	if($sth == true) { ?>
  	          <script type="text/javascript">
  			  alert('Data Successfully Updated');
  			  window.location="curriculum.php?id=<?php echo $refid;?>";
  			</script>
   ...
   if(isset($_POST) && $_POST['action']=='delete') {
	...
	//print_r($_POST); exit;
  	$d = $_POST['guid'];
  	$sth = $db->query("DELETE FROM `curriculum` WHERE `guid`='".$d."'");
  	if($sth == true) 
  	{
  	 echo "1111";	 exit;
  	} 
...
event.preventDefault();
	...
	//console.log("called");
   $(".topics").append('<label>Topic Name</label><input type="text" name="name[]" placeholder="Enter Topic Here" id="name" class="form-control" required=""><label>Course Duration Time</label><input type="text" name="dur[]" placeholder="Enter Topic Here" id="dur" class="form-control" required="">');
  ...
  <body class="sidebar-mini fixed">
    <div class="wrapper">
      ...
      <div class="content-wrapper">
		...
		<a href="<?php echo URL; ?>curriculum.php?id=<?php echo $_GET[id]; ?>" class="btn btn-info">Back</a>
        <div class="row">
		
		<div class="col-md-6">
            <div class="card">
              <h3 class="card-title">Add New Curriculum Title</h3>
              <div class="card-body">
                <form method="post" action="<?php echo $_SEVER['PHP_SELF'];?>" enctype="multipart/form-data" onsubmit="return validate();">
				  ...
				  <div class="form-group topics">
                    <label class="control-label">Curriculum Title</label>
                    <input type="text" name="name[]" placeholder="Enter Curriculum Title Here" id="name" class="form-control" required>
					
					 <label class="control-label">Course Duration Time</label>
                    <input type="text" name="dur[]" placeholder="Enter Course Duration Time Here" id="dur" class="form-control" required>
					</br>
	   ...
	   $q = $_GET['guid'];
	   $sth = $db->query ('SELECT * FROM `curriculum` WHERE `guid`="'.$id.'"');
	   $row = $sth->fetch();
	   ?>
              ...
              <div class="card-body">
                <form method="post" action="<?php echo $_SEVER['PHP_SELF'];?>" enctype="multipart/form-data">
				 ...
				 <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control" id="name" value="<?php echo $row[2];?>" >
                  </div>
				  <div class="form-group">
                    <label class="control-label">Course Duration Time</label>
                    <input type="text" name="dur" class="form-control" id="dur" value="<?php echo $row[3];?>" >
                  </div>
	  ...
	  $q = $_GET['guid'];
	  $id = $_GET['id'];
	  $vth = $db->query ("DELETE FROM `curriculum` WHERE `guid`='".$q."'" );
	  
	  if($vth==true)
	  {?>
	  <script type="text/javascript">
			alert('Data Successfully Deleted');
			window.location="<?php echo URL; ?>curriculum.php?id=<?php echo $id; ?>";
			</script>
	  <?php  } else { ?>
	<script type="text/javascript">
			alert('Please try Again');
	 ...
	 // $q = $_GET['guid'];
	 // $vth = $db->query ("DELETE FROM `curriculum` WHERE `guid`='".$q."'" );
	 // header('location:'.URL.'curriculum.php'); 
		  ...
		  <a href="curriculum.php?id=<?php echo $_GET[id];?>" class="btn btn-default"><!--i class="fa fa-lg fa-plus"></i-->Curriculum For This Course</a>
		  ...
		  <!--a href="curriculum.php?action=add" class="btn btn-success"><i class="fa fa-lg fa-plus"></i>Add New</a><!--a href="curriculum.php?action=edit" class="btn btn-info btn-flat"><i class="fa fa-lg fa-edit"></i></a><a href="#" class="btn btn-warning btn-flat"><i class="fa fa-lg fa-trash"></i></a-->
		  </br>
		  </br>
		  </div>
		  ...
		  <?php
            $sth = $db->query ("SELECT * FROM `curriculum` WHERE id='$id'");
			$count = $sth->rowCount(); ?>
            <div class="card">
              <div class="card-body">
                <table id="sampleTable" class="table table-hover table-bordered">
                  <thead>
				  <tr><a href="curriculum.php?action=add&id=<?php echo $_GET[id];?>" class="btn btn-success"> <i class="fa fa-lg fa-plus"></i> Add Curriculum For This Course</a> </br></br> </tr>
                    <tr>
                      <th>S No.</th>
					  <!--th>Image</th-->
                      <th>Curriculum Titles</th>
                      <th>Course Duration Time</th>
					  <th>Add Data Here</th>
                      <!--th> Sub Title</th>
                      <th>WBF</th>
				  ...
				  <?php 
                    ...
                    <tr>
					  ...
					  <td><?php   echo substr($row[2],0,55);?></td>
					  ...
					  <td> 
					   ...
					   <div><a href="chapters.php?id=<?php echo $row[0]; //$_GET[refid];?>&refid=<?php echo $row[1]; //$_GET[refid];?>" class="btn btn-success"><i class="fa fa-lg fa-plus"></i>Add Chapters</a>
					  ...
					  <a href="<?php echo URL; ?>curriculum.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> 
					  </div-->
					  </td>
                      
                      <!--td><img src="adminupload/<?php echo $row[5]; ?>" height="100" width="100" alt="" /></td-->
					  <td class="center"><div class="btn btn-xs btn-group-xs"><a href="curriculum.php?action=edit&id=<?php echo $row[0]; ?>&refid=<?php echo $row[1]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> 
					  
					 <a href="<?php echo URL; ?>curriculum.php?action=deleteData&guid=<?php echo $row[0]; ?>&id=<?php echo $row[1]; ?>" class="btn btn-danger" onClick="return delete1();">Delete </a> </div></td>
                    </tr>
	  <?php $m++; } } ?>
                  </tbody>
                </table>
