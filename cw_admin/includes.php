...
if(isset($_POST) && $_POST['submit']=='ADD') {
	...
	for($i=0;$i<sizeof($_POST['name']);$i++){
		...
		//echo "INSERT INTO `tutorial`(`id`,`name`) VALUES ('$id','".$_POST[name][$i]."')";exit;
		 ...
		 $sth = $db->query("INSERT INTO `includes`(`id`,`name`) VALUES ('$id','$ans')");
	}
	if($sth == true)  { ?>
  	        <script type="text/javascript">
  			alert('Data Successfully Inserted');
  			window.location="includes.php?id=<?php echo $id; ?>";
  			</script>
  ...
  if(isset($_POST) && $_POST['submit']=='EDIT')
  {
   ...
   // if(!empty($filename)) 
	//{
	        ...
	        //$a1 = $db->query("UPDATE `includes` SET `image`= '$image' WHERE `guid`='$qid'");
	// }
	...
	if($name!='')
	{
		...
		$ans = str_replace('"',"~~", $an);
		//echo $refid;exit;
		//echo "UPDATE `includes` SET `description`= '$ans' WHERE `guid`='$guid'";exit;
		//echo "UPDATE `includes` SET `name`= '$name',`sbttl`= '$designation',`wbf`= '$ans' WHERE `guid`='$qid'";exit;
	$sth = $db->query("UPDATE `includes` SET `name`= '$ans' WHERE `guid`='$qid'");
	}
	
	if($sth == true) { ?>
  	          <script type="text/javascript">
  			  alert('Data Successfully Updated');
  			  window.location="includes.php?id=<?php echo $refid;?>";
  			</script>
   ...
   if(isset($_POST) && $_POST['action']=='delete') {
	...
	//print_r($_POST); exit;
  	$d = $_POST['guid'];
  	$sth = $db->query("DELETE FROM `includes` WHERE `guid`='".$d."'");
  	if($sth == true) 
  	{
  	 echo "1111";	 exit;
  	} 
  ...
  <body class="sidebar-mini fixed">
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        <div class="page-title">
          <div>
            ...
            <ul class="breadcrumb">
              ...
              <li class="active"><a href="#">includes</a></li>
            </ul>
          </div>
		...
		<a href="<?php echo URL; ?>includes.php?id=<?php echo $_GET[id]; ?>" class="btn btn-info">Back</a>
        <div class="row">
	   ...
	   $q = $_GET['guid'];
	   $sth = $db->query ('SELECT * FROM `includes` WHERE `guid`="'.$id.'"');
	   $row = $sth->fetch();
	   ?>
	   
		  <div class="col-md-6">
		  <a href="<?php echo URL; ?>includes.php?id=<?php echo $_GET[refid]; ?>" class="btn btn-info">Back</a>
            <div class="card">
              <h3 class="card-title">Edit includes</h3>
              <div class="card-body">
	  ...
	  $q = $_GET['guid'];
	  $id = $_GET['id'];
	  $vth = $db->query ("DELETE FROM `includes` WHERE `guid`='".$q."'" );
	  
	  if($vth==true)
	  {?>
	  <script type="text/javascript">
			alert('Data Successfully Deleted');
			window.location="<?php echo URL; ?>includes.php?id=<?php echo $id; ?>";
			</script>
	  <?php  } else { ?>
	<script type="text/javascript">
			alert('Please try Again');
	 ...
	 // $q = $_GET['guid'];
	 // $vth = $db->query ("DELETE FROM `includes` WHERE `guid`='".$q."'" );
	 // header('location:'.URL.'includes.php'); 
		  ...
		  <a href="requirements.php?id=<?php echo $_GET[id];?>" class="btn btn-success"><i class="fa fa-lg fa-plus"></i> Requirements</a>
		 
		  <a href="includes.php?id=<?php echo $_GET[id];?>" class="btn btn-default"><!--i class="fa fa-lg fa-plus"></i-->Includes</a>
		  <!--a href="includes.php?action=add" class="btn btn-success"><i class="fa fa-lg fa-plus"></i>Add New</a><!--a href="includes.php?action=edit" class="btn btn-info btn-flat"><i class="fa fa-lg fa-edit"></i></a><a href="#" class="btn btn-warning btn-flat"><i class="fa fa-lg fa-trash"></i></a-->
		  </br>
		  </br>
		  </div>
		  ...
		  <?php
            $sth = $db->query ("SELECT * FROM `includes` WHERE id='$id'");
			$count = $sth->rowCount(); ?>
            <div class="card">
              <div class="card-body">
                <table id="sampleTable" class="table table-hover table-bordered">
                  <thead>
				  <tr><a href="includes.php?action=add&id=<?php echo $_GET[id];?>" class="btn btn-success"> <i class="fa fa-lg fa-plus"></i> Add Includes</a> </br></br> </tr>
                    <tr>
                      <th>S No.</th>
					  <!--th>Image</th-->
                      <th>Topics</th>
				  ...
				  <?php 
                    ...
                    <tr>
					  ...
					  <a href="<?php echo URL; ?>includes.php?action=deleteData&guid=<?php echo $row[0]; ?>" class="btn btn-danger" onClick="return delete1();"><i class="fa fa-trash icon-only"></i></a> 
					  </div-->
					  </td>
                      
                      <!--td><img src="adminupload/<?php echo $row[5]; ?>" height="100" width="100" alt="" /></td-->
					  <td class="center"><div class="btn btn-xs btn-group-xs"><a href="includes.php?action=edit&id=<?php echo $row[0]; ?>&refid=<?php echo $row[1]; ?>"  class="btn btn-inverse"><i class="fa fa-edit icon-only"></i></a> 
					  
					 <a href="<?php echo URL; ?>includes.php?action=deleteData&guid=<?php echo $row[0]; ?>&id=<?php echo $row[1]; ?>" class="btn btn-danger" onClick="return delete1();">Delete </a> </div></td>
                    </tr>
	  <?php $m++; } } ?>
                  </tbody>
                </table>
